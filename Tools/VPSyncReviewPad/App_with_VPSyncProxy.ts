...
export class App extends Server {
  ...
  constructor() {
    ...
    this.app.post('/vpsync/review-drawing', async (req, res) => {
  try {
    const drawingData = req.body.DrawingData;

    await request
      .put(
        'http://127.0.0.1:30010/remote/preset/' +
        encodeURIComponent('RC_VPSyncOperator') +
        '/function/' +
        encodeURIComponent('Receive Review Drawing')
      )
      .send({
        Parameters: {
    ...
    console.error('VPSync Review Drawing Error:', error.message);
